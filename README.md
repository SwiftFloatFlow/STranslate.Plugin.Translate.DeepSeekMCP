...
DeepSeekMCP 的核心优势是支持 AI 在翻译过程中**动态调用外部 MCP 服务器工具**，显著提升回复质量：
...
1. **技术文档翻译**：AI 自动查询术语定义、API 文档，确保专业术语准确
2. **实时信息翻译**：调用搜索引擎获取最新背景信息，提供上下文感知翻译
3. **多语言术语统一**：连接企业术语库，保持翻译一致性
4. **数据驱动翻译**：查询数据库获取产品规格、用户信息等，准确翻译数据相关内容
...
- MCP 服务功能关闭时，MCP 翻译功能不可用，但策略编辑和命令系统**独立运行**
- 提示词独立绑定策略，互不干扰
...
- 命令以 `/` 开头，触发命令系统
- 语言需一致（中文或英文），如 `/切换 工具强制` 或 `/switch hybrid`
- **命令系统独立于 MCP 服务功能**：MCP 禁用后，命令系统仍可正常使用
- `/工具链`、`/工具结果`命令需要选择提示词后才可生效
...
| 上限类型 | 默认值 | 最小值 | 最大值 | 特殊值 |
| -------- | ------ | ------ | ------ | ------ |
| 总工具调用上限 | 15次 | -1 | 50 | -1=无上限，0=禁用 |
| 同一工具连续调用上限 | 5次 | -1 | 10 | -1=无上限，0=禁用 |
...
SaveSettings() → Context.SaveSettings()
    ↓
保存到 STranslate\current\PortableConfig\Settings\Plugins\STranslate.Plugin.DeepSeek.MCP_d99c702e39b44be5a9e49983ff0f4fff
```
...
2. 在 `Main.cs` 的 `GetSystemPromptByStrategy()` 添加提示词
3. 在 `Converters/StrategyConverters.cs` 添加名称和描述
4. 在 `SettingsView.xaml` 更新策略下拉框绑定
5. 在 `Main.cs` 的 `ExecuteCommandAsync()` 添加策略名称映射

### 场景2：修改工具链显示格式

1. 找到 `Main.cs` 中的 `FormatToolChainItem()` 方法
2. 修改返回格式（当前：`toolName✅` / `toolName❌`）
3. 修改 `TranslateWithMcpTools()` 中的显示逻辑

### 场景3：调整 UI 布局

1. 打开 `View/SettingsView.xaml`
2. 修改 Grid 的 ColumnDefinition 宽度（标签列默认 110px）
3. 调整 Margin 值（标签右边距默认 20px）
4. 确保所有行的列定义保持一致
...
1. 查看 `docs/guides/troubleshooting.md` 故障排除指南
2. 检查 `STranslate\current\PortableConfig\Logs` 中的日志
3. 提交 Issue 时提供（因作者使用 AI 制作本插件项目，故不保证及时处理问题）：
   - 环境信息（Windows/.NET 版本）
   - 复现步骤
   - 相关日志片段
   - 配置文件（删除敏感信息）
